...
│   └── copy-stockfish.mjs              [P0]    postinstall: copy lite-single .js/.wasm + GPL text into public/stockfish
│
├── public/
...
│   ├── stockfish/                              (generated by scripts/copy-stockfish.mjs; git-ignored)
...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      {
        // filenames are version-stamped (stockfish-18-lite-single.*) -> safe forever
...
await copyFile(join(from, "bin/stockfish-18-lite-single.js"), join(to, "stockfish-18-lite-single.js"));
await copyFile(join(from, "bin/stockfish-18-lite-single.wasm"), join(to, "stockfish-18-lite-single.wasm"));
await copyFile(join(from, "Copying.txt"), join(to, "LICENSE-GPL-3.0.txt")); // GPLv3 compliance
console.log("[stockfish] lite-single build copied to public/stockfish");
```

`.gitignore` — [P0] append `/public/stockfish/`. (`.eve/` and `.output/` are already ignored.)
...
3. `aiStore.setPhase("engine")`. `use-stockfish` lazily creates the worker
   (`new Worker("/stockfish/stockfish-18-lite-single.js")` — a **classic, same-origin worker loaded
   by URL string from `public/`**; never bundled through Turbopack, never
   `new Worker(new URL(...))`). First use downloads 5.6 MB gz; the progress `MessagePort`
   drives `engine-loading.tsx`. The `.wasm` is resolved automatically as the sibling of the `.js`.
...
8. The handler streams NDJSON back: `{"t":"delta","d":"…"}` for each `message.appended`
   `messageDelta`, then one `{"t":"result","d":{move,commentary,source,eveSessionId}}`.
...
10. The client reads the stream. Deltas go to `aiStore.appendCommentary` and render live in
    `commentary-panel` (FR-37). On `result`:
...
| response | `Content-Type: application/x-ndjson`, one JSON object per line, terminated by exactly one `{"t":"result"}` frame. |
...
- [ ] `pnpm install` runs `scripts/copy-stockfish.mjs` and `public/stockfish/` contains the `.js`, `.wasm` and `LICENSE-GPL-3.0.txt`.
...
- [ ] Pieces are procedural (`LatheGeometry` profiles from `assets.md` §B3 + extruded knight); geometries/materials memoised and shared across the 32 pieces.
...
1. **NFR-3 "Stockfish WASM under 2 MB gzipped" is not achievable and is formally relaxed.**
   The smallest build the installed `stockfish@18.0.8` ships is `lite-single` at **5.64 MB gzipped**
   (measured). No sub-2 MB artifact exists in the tarball; an `ULTRA_LITE_NET` build would require
   emscripten 3.1.7. **Decision:** ship `stockfish-18-lite-single` from `/public`, lazily, only in AI
   mode, with a visible download progress bar and `Cache-Control: immutable`. Revised NFR-3:
   *"≤ 6 MB gzipped, lazy-loaded in AI mode only, cached for a year."* Single-threaded also means
   **no COOP/COEP**, which is what keeps Clerk, Convex and cross-origin assets working.
   The engine is GPLv3 — `LICENSE-GPL-3.0.txt` ships alongside it and the file banners stay intact.
...
7. **No CC0 chess GLB exists** (PRD §1 assumed one). Every chess model on Poly Pizza is CC-BY 3.0 and
   Sketchfab requires a login. **Decision:** procedural Staunton pieces built from
   `THREE.LatheGeometry` profiles (already authored, with exact radius/height tables, in
   `assets.md` §B3) plus an extruded knight head, a merged rook crenellation, a king cross and queen
   crown orbs. Benefits: zero download, one shared geometry per piece type, materials driven by the
   room preset (FR-26). Cost: they need a visual pass; the profiles are proportions, not a scan.
   The CC-BY Jarlan Perez set is a documented fallback if attribution becomes acceptable.
