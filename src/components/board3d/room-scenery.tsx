...
const skyFragment = /* glsl */ `
  ...
  void main() {
    ...
    vec3 base = vec3(.018, .033, .071);
    vec3 nebula = mix(vec3(.06, .29, .38), vec3(.27, .16, .39), smoothstep(-.8, .8, d.x));
    vec3 color = base + nebula * band * (.3 + dust * 1.4);
    color += vec3(.28, .31, .38) * pow(filaments, 3.0) * band;
...
export function NeonStage() {
  return (
    <group>
      ...
      <hemisphereLight args={["#c3dfff", "#44415e", 0.8]} />
